...
/**
 * This actor conusmes from the Message Queue and sends the messages to the system.
 */
object MqConsumer:
  def apply(
    channel: Channel,
  ...
  /**
   * This behavior consumes messages from the Message Queue. Since the "basicConsume" method is
   * blocking, the behavior remains blocked until the connection is closed but the actor still sends
   * the messages to the system.
   *
   * @param channel
